...
<body>
	<div class="container-fluid">
		<div class="d-sm-flex align-items-center justify-content-between mb-4">
			<h1 class="h3 mb-0 text-gray-800">매출 등록</h1>
		</div>
		<hr class="my-4">
		...
		<form id="salesForm" action="/owner/detail/add" method="post">
				...
				<div class="col-md-3">
					<div class="input-group date" id="datepicker">
						...
						<input type="text" class="form-control custom-date"
							name="salesDate" id="salesDate" readonly
			...
			<input type="hidden" id="salesData" name="salesData">
			<button type="button" id="addRow" class="btn btn-secondary">제품
				추가</button>
		...
		$(document)
				.ready(
						function() {
									...
									.submit(
											function(e) {
												...
												$('#salesData')
														.val(
																JSON
																		.stringify(salesDataList));
												this.submit();
											});
							//행추가
		                     $('#addRow').click(function() {
		                        var newRow = $('.salesData:last').clone();
		                     ...
		                     $('#removeRow').click(function() {
		                          if ($('.salesData').length > 1) {
		                              $('.salesData:last').remove();
		                          } else {
		                              alert('최소 1개 제품은 필요합니다.');
		                          }
		                      });

							bindEventsToRow($('.salesData'));
							...
							// salesDate 필드 클릭 시 날짜 선택기 표시
							$('#salesDate').on('click', function() {
